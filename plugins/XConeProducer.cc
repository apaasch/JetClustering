  ...
  for(unsigned int i=0; i<fatjets.size(); i++){
    ...
    if (enoughParticles && doSubjets) {
      ...
      for (unsigned int j = 0; j < subjets.size(); ++j) {
        ...
        // Nsubjettiness subjets
        double beta = 0.1;
        Nsubjettiness nSub1(1, OnePass_WTA_KT_Axes(), NormalizedMeasure(beta, 0.4));
        Nsubjettiness nSub2(2, OnePass_WTA_KT_Axes(), NormalizedMeasure(beta, 0.4));
        Nsubjettiness nSub3(3, OnePass_WTA_KT_Axes(), NormalizedMeasure(beta, 0.4));
        double tau1 = nSub1(subjets[j]);
        double tau2 = nSub2(subjets[j]);
        double tau3 = nSub3(subjets[j]);

        subjet.setJetArea(subjet_area[j]);
        subjet.addUserFloat("tau1", tau1);
        subjet.addUserFloat("tau2", tau2);
        subjet.addUserFloat("tau3", tau3);
    ...
    // Nsubjettiness
    double beta = 1.0;
    Nsubjettiness nSub1(1, OnePass_WTA_KT_Axes(), NormalizedMeasure(beta, 1.2));
    Nsubjettiness nSub2(2, OnePass_WTA_KT_Axes(), NormalizedMeasure(beta, 1.2));
    Nsubjettiness nSub3(3, OnePass_WTA_KT_Axes(), NormalizedMeasure(beta, 1.2));
    double tau1 = nSub1(fatjets[i]);
    double tau2 = nSub2(fatjets[i]);
    double tau3 = nSub3(fatjets[i]);
    ...
    patJet.addUserFloat("tau1", tau1);
    patJet.addUserFloat("tau2", tau2);
    patJet.addUserFloat("tau3", tau3);
